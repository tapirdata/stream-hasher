...
function makeTests(title: string, options: SingleTestOptions) {
  describe(title, () => {
    ...
    before((done) => {
      ...
      return fs.createReadStream(path.join(__dirname, options.relPath)).pipe(hasher).on('end', done).resume();
    });

    return it('should emit the correct hash', () => expect(digest).to.be.equal(expectedDigests[options.relPath]));
  });
}

describe('stream-hasher for single stream', () => {
  makeTests('with short file', { relPath: 'fixtures/eels.txt' });

  return makeTests('with long file', { relPath: 'fixtures/lorem.txt' });
});
